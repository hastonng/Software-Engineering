...
public class setGPAvailabilityPage 
{
	...
	public static void show()  
	{
		...
		panel_2.setBounds(135, 30, 1103, 685);
		panel_2.setBorder(new LineBorder(new Color(0, 0, 0), 3, true));
		panel.add(panel_2);
		panel_2.setLayout(null);
		
		JLabel lblNewLabel_2 = new JLabel("Set General Pratitioner Availability");
        lblNewLabel_2.setFont(new Font("Verdana", Font.BOLD, 30));
        lblNewLabel_2.setBounds(275, 0, 598, 47);
        panel_2.add(lblNewLabel_2);
        
		
		JLabel lblGP_ID = new JLabel("Employee ID:");
		lblGP_ID.setFont(new Font("Tahoma", Font.PLAIN, 25));
		lblGP_ID.setBounds(26, 58, 165, 30);
		panel_2.add(lblGP_ID);
		
        JTextField gpIDField = new JTextField();
        gpIDField.setFont(new Font("Tahoma", Font.PLAIN, 24));
        gpIDField.setBounds(187, 58, 450, 30);
        panel_2.add(gpIDField);
        gpIDField.setColumns(10);
        gpIDField.addKeyListener(new KeyAdapter() {
			@Override
			public void keyTyped(KeyEvent e) {
				if (gpIDField.getText().length() >= 8)
					e.consume();
			}
		});
				...
				try
					{
						...
						{
							emp.generateId();
							emp.setGpAvailable(gpIDField.getText(), startDate, endDate, startTime, endTime);
							emp.clear();
						}
